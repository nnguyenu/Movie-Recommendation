...
class CF(object):
    ...
    def recommend(self, u, normalized = 1, top = 10):
        """
        ...
        return [_[0] for _ in recommended_items[:top]]
...
def predict(id,rate):
    new_rate_train = rate_train.copy()
    user_id = np.max(new_rate_train[:,0])+1
    for i in range(len(id)):
        new_rate = np.array([[user_id,id[i],rate[i],1]])
        new_rate_train = np.append(new_rate_train,new_rate,axis = 0)
    new_rate_train[:, :2] -= 1
    rs = CF(new_rate_train, k = 30, user_CF = 1)
    rs.fit()
    rec = rs.recommend(rs.n_users - 1)
    rec = [_ + 1 for _ in rec]
    return rec